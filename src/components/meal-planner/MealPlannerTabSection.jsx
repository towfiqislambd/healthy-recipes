...
const MealPlannerTabSection = () => {
  ...
  const { data: recipeLibrary, isLoading: isRecipeLibraryLoading } =
    useRecipeLibrary();
  const { data: allRecipes, isLoading: loadingAllRecipe } = useAllRecipes(
    activeTab?.id,
    library,
    ageGroup,
    null,
  ...
  const {
    data: recipesPrivate,
    refetch,
    isLoading: privateRecipesLoading,
  } = useAllRecipesPrivate(activeTab?.id, library, ageGroup, null, search);
